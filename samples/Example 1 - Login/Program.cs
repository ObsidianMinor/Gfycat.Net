...
namespace LoginSample
{
    class Program
    {
        ...
        async Task SimpleClientAuthenticationAsync()
        {
             ...
             * you can't access current user endpoints with this authentication method
             */

            await client.AuthenticateAsync(); // used to perform a simple client authentication
        ...
        async Task PasswordAuthAsync()
        {
            ...
            await client.AuthenticateAsync("USERNAME_OF_ACCOUNT_WHICH_OWNS_CLIENT", "PASSWORD_OF_ACCOUNT_WHICH_OWNS_CLIENT");
        }

        async Task ImplicitAuthAsync()
        {
            GfycatClient client = new GfycatClient(_clientId, _clientSecret);

            string authUrl = client.GetBrowserAuthUrl("randomState", "http://example.com/oauth/callback", false);
        ...
        async Task AuthCodeAsync()
        {
            ...
            string authUrl = client.GetBrowserAuthUrl("RANDOMstate", "http://example.com/oauth/callback", true);
            ...
            await client.AuthenticateAsync(TokenType.AuthorizationCode, "code", "http://example.com/oauth/callback");
        }
        ...
        async Task FacebookAuthAsync()
        {
            ...
            await client.AuthenticateAsync(TokenType.FacebookAuthCode, "authCode");
        }
        ...
        async Task FacebookTokenAsync()
        {
            ...
            await client.AuthenticateAsync(TokenType.FacebookAccessToken, "token!");
        }
        ...
        async Task TwitterAsync()
        {
            ...
            string requestToken = await client.GetTwitterRequestTokenAsync();
            ...
            await client.AuthenticateAsync(TokenType.TwitterTokenVerifier, requestToken, "verifier");
        }
